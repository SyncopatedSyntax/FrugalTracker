...
| U11 | Analytics depth over time | Insights was strong on the current period but thin on *trends*. A product-review roadmap (approved) adds, in order: **category detail screen** (12-mo trend + MoM/YoY — resolved v1.1.0, `CategoryDetailScreen.tsx` at `/insights/category/:id`, helpers `monthlySeriesFor`/`deltaVs` in `compute.ts`), a period **savings-rate** stat (resolved v1.2.0 — `savingsRate()` in `compute.ts`, shown on the Overview view under Income/Expenses), a **budget-views** package (all-budgets overview, sort/filter, cumulative-YTD pace, budget history), and a **calendar heatmap** Insights view (resolved v1.3.0 — `CalendarHeatmap.tsx`, a 4th "Calendar" Insights view; helper `dailyTotals()` in `compute.ts`; month-navigating grid shaded by daily expense, honors `firstDayOfWeek`, tap a day → its transactions). Tag/label detail parity and accounts/transfers are explicitly out of scope for now. Remaining: the budget-views package (Phase 3). |
